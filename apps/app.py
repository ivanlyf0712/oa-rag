...
from apps.search_cli import (
    build_aggregate_tool, build_compare_tool, build_contract_tool,
    build_detail_tool, build_where_tool,
)
try:
...
def _build_agent(index_path: str, embeddings):
    ...
    detail_tool = build_detail_tool(embeddings)
    compare_tool = build_compare_tool(embeddings)
    return _build_search_agent(
        contract_tool, where_tool, _memory(), aggregate_tool,
        detail_tool, compare_tool, notify=st.caption)
